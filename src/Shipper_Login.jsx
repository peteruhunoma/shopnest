...
function Shipper_Login() {
  ...
  const {sellerLogin, currentuser} = useContext(AuthContext);
...
try{
  ...
  const res = await sellerLogin({emailOrUsername, password});
  res;
  console.log(res);
  if(res.status == 200){
    history.push("/");
...
console.log(err);
  ...
  return (
<div className="fontmsu bg-background-light dark:bg-background-dark font-display text-gray-800 dark:text-gray-200">
...
<h1 className="text-2xl font-bold text-gray-900 dark:text-white"> Login to Seller Central</h1>
